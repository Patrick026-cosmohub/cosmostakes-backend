...
import { listAnnouncements, upsertAnnouncement, deleteAnnouncement, listGamesAdmin } from "@/lib/portal.functions";
...
function AnnouncementsPage() {
  ...
  const save = useMutation({
    ...
    onSuccess: () => { toast.success("Saved"); setEditing(null); qc.invalidateQueries({ queryKey: ["announcements"] }); },
  ...
  return (
    <div className="p-4 lg:p-6 space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold flex items-center gap-2"><Megaphone className="size-5 text-primary" /> Announcements</h1>
          <p className="text-xs text-muted-foreground">Schedule, pin and broadcast platform-wide messages.</p>
        </div>
        <Button onClick={() => setEditing({ is_active: true })}><Plus className="size-4 mr-1" /> New</Button>
      </div>
          ...
          <Card key={a.id}>
            <CardContent className="p-4 flex items-start gap-3">
              ...
              <div className="flex gap-1.5">
                <Button size="sm" variant="outline" onClick={() => setEditing(a)}><Pencil className="size-3" /></Button>
                <Button size="sm" variant="ghost" onClick={() => remove.mutate(a.id)}><Trash2 className="size-3 text-destructive" /></Button>
              </div>
            </CardContent>
          </Card>
        ))}
        {rows.length === 0 && !q.isLoading && <Card><CardContent className="py-10 text-center text-sm text-muted-foreground">No announcements yet.</CardContent></Card>}
      </div>
          ...
          {editing && (
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div><Label>Ends at</Label><Input type="datetime-local" value={editing.ends_at?.slice(0, 16) ?? ""} onChange={(e) => setEditing({ ...editing, ends_at: e.target.value || null })} /></div>
              </div>
              <div>
              ...
              <div className="flex gap-4">
                <div className="flex items-center gap-2"><Switch checked={editing.pinned ?? false} onCheckedChange={(v) => setEditing({ ...editing, pinned: v })} /><Label>Pinned</Label></div>
                <div className="flex items-center gap-2"><Switch checked={editing.push_enabled ?? false} onCheckedChange={(v) => setEditing({ ...editing, push_enabled: v })} /><Label>Push</Label></div>
