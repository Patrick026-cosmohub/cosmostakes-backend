    ...
    <div className="p-4 lg:p-6 space-y-5">
      <div>
        ...
        <p className="text-xs text-muted-foreground">
          Live infrastructure & integration health (auto-refresh 15s).
        </p>
      </div>
        ...
        <>
          <div className="grid gap-3 md:grid-cols-3">
            ...
            <HealthCard
              name="Storage"
              ok={d.storage.ok}
              latency={d.storage.latencyMs}
              note={d.storage.error}
            />
          </div>
          ...
          <Card>
            <CardContent className="p-5">
              ...
              <ul className="divide-y divide-border/50">
                {(
                  d.integrations as unknown as Array<{
                    id: string;
                  ...
                  <li key={i.id} className="py-2 flex items-center gap-3 text-sm">
                    ...
                    <span className="ml-auto text-xs text-muted-foreground">
                      {i.last_test_at ? fmtRelative(i.last_test_at) : "never tested"}
                    </span>
                  </li>
                ))}
                {(d.integrations as unknown[]).length === 0 && (
